...
packagetable <- data.frame(
  ...
  col3 = c("no", "yes", "no", "no", "no", "no", "yes", "no", "no", "no", "yes", "yes"),
...
knitr::kable(packagetable, col.names = c("Package name", "Summary", "GUI available", "Supports GLMMs", "Design limitations", "Reference"), escape = FALSE)

```

# Reasons to use tailored data simulation

Performing tailored simulation-based sample size planning is more complicated and time-consuming than using the existing software tools outlined in @tbl-rpackage. Circumstances under which tailored simulation-based sample size planning is necessary include 1) complex study designs, 2) complex statistical hypotheses, 3) planning for precision, and 4) lack of prior studies or pilot data.
...
We can use plots like @fig-margdist1 as a useful tool to decide whether the specified standard deviations of the subject and item random intercepts ($\sigma_S$ and $\sigma_I$) are reasonable by comparing the ranges and overlap between conditions to domain knowledge.
