...
The majority of empirical studies in psychology apply hypothesis testing. As a consequence, the dominant approach for sample size planning is based on power analysis (i.e., planning for power). However, sample size planning can also be based on the precision of parameter estimates (i.e., planning for precision). We will now discuss both concepts. 
...
Not all research questions are best answered using hypothesis testing. Especially for exploratory studies, when little previous research has been conducted, only estimating the effects of interest is often more useful. When no hypothesis tests are conducted for the planned study, power analysis is not available for sample size planning. Nonetheless, the sample size still has a crucial effect on how informative the planned study will be to learn about the effect of interest. In this case, the expected width of a confidence interval is used as a target for sample size planning. The values inside a confidence interval are commonly interpreted as plausible values for the quantity of interest the confidence interval is supposed to estimate. More formally, a 95%-confidence interval provides the smallest interval with the property that upon repeated sampling, 95% of individual confidence intervals would include the true quantity of interest. Thus, a narrow confidence interval is more informative about the size of the true effect than a wide interval. Apart from the desired confidence level, the width of a confidence interval depends strongly on the sample size. Because bigger samples carry more information, they lead to smaller confidence intervals. When planning for precision, a target is set for the expected width of a confidence interval of interest. Assuming some effect size of interest and a desired confidence level, a minimum sample size can be determined that, on average, would guarantee reaching this target.

## Generalized linear mixed models (GLMMs)

As study designs become more complex, psychological researchers require more sophisticated statistical models to capture the nuanced relationships and grouping structures introduced by them [@yarkoniGeneralizabilityCrisis2022]. GLMMs (also called multilevel models) are gaining increasing popularity because they offer a flexible framework for handling data with outcome variables that are not normally distributed (e.g., categorical outcomes) while accounting for both fixed and random effects [@fahrmeirRegressionModelsMethods2021].
...
All code in this manuscript and simulation results are available in the project's repository on the Open Science Framework (https://osf.io/dhwf4/).
