...
The most prominent heuristic is to target a power of $1- \beta = 0.8$ in combination with a type I error rate of $\alpha = 0.05$ [@lakensSampleSizeJustification2022]. However, depending on the research goals or resource constraints, there are often good reasons to move away from this standard [@benjaminRedefineStatisticalSignificance2017; @lakensJustifyYourAlpha2018]. For example, the *Social Sciences Replication Project* targeted a power of 0.9 to safeguard against biased effect sizes in the original studies [@camererEvaluatingReplicabilitySocial2018].
...
packagetable <- data.frame(
  ...
  col6 = c("@watsonGeneralisedLinearMixed2023", "@iddiPowerSampleSize2022", "@kumleEstimatingPowerGeneralized2021", "@endersSimpleMonteCarlo2023", "@martinMeasuringIndividualDifferences2011", "@marcochakyanyuPassLmePower2019", "@lafitSelectionNumberParticipants2021", "@magnussonConsequencesIgnoringTherapist2018", "@lebeauSimglmSimulateModels2017", "@greenSIMRPackagePower2016", "@murayamaSummarystatisticsbasedPowerAnalysis2022", "@zhangPracticalStatisticalPower2018")
)

knitr::kable(packagetable, col.names = c("Package name", "Summary", "GUI available", "GLMM support", "Design limitations", "Reference"), escape = FALSE)
```

# Reasons to use tailored data simulation

Performing tailored simulation-based sample size planning is more complicated and time-consuming than using the existing software tools outlined in @tbl-rpackages. Circumstances under which tailored simulation-based sample size planning is necessary include 1) complex study designs, 2) complex statistical hypotheses, 3) planning for precision, and 4) lack of prior studies or pilot data.
...
None of the software packages for sample size planning in @tbl-rpackages can handle combined hypotheses and only some can handle directed hypotheses. In contrast, our case study will demonstrate how we can test directed combined hypotheses with tailored simulation-based sample size planning. 

<!-- ## Planning for precision -->

Third, all available software packages for sample size planning with GLMMs are based on power analysis and do not support planning for precision. Thus, researchers that want to apply an estimation strategy instead of testing statistical hypotheses [@cummingNewStatisticsWhy2014], currently cannot use the software packages outlined in @tbl-rpackages.
...
We further validated the plausibility of our estimated gains and losses with the collaborating radiologists until we settled on the probabilities shown in @tbl-probtable.
