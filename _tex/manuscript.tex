...
al., 2018}). For example, the \emph{Social Sciences Replication Project}
targeted a power of 0.9 to safeguard against biased effect sizes in the
original studies
(\citeproc{ref-camererEvaluatingReplicabilitySocial2018}{Camerer et al.,
2018}).
...
have exclusively focused on planning for power. In
Table~\ref{tbl-rpackages}, we review available software packages that
can be used to perform power analysis for multilevel models in R
{[}Version 4.4.0; R Core Team (\citeproc{ref-R-base}{2024}){]}. Power
analysis methods can be categorized into formula-based,
summary-statistics-based and simulation-based methods
...
2016a}; \citeproc{ref-kainPracticalGuidePower2015}{Kain et al., 2015};
\citeproc{ref-kumleEstimatingPowerGeneralized2021}{Kumle et al., 2021};
...
However, many of these tutorials focus on LMMs and the most common study
designs (see \citeproc{ref-kumleEstimatingPowerGeneralized2021}{Kumle et
al., 2021} for a tutorial that also covers more advanced settings). This
narrow focus provides limited guidance for researchers using more
complex study designs, especially when little prior knowledge about
plausible effect sizes is available (see the discussion in
\citeproc{ref-kumleEstimatingPowerGeneralized2021}{Kumle et al., 2021}).
...
{\caption{{Overview of software packages for power analysis for
(generalized) linear mixed models in R.}{\label{tbl-rpackages}}}
\vspace{-20pt}}

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1165}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.3947}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0526}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0489}}
...
\bottomrule\noalign{}
\endlastfoot
glmmrBase & Formula- and simulation-based power analysis; non-linear
fixed effects and flexible covariance functions & no & yes & none &
Watson (\citeproc{ref-watsonGeneralisedLinearMixed2023}{2023}) \\
longpower & Formula-based power analysis; focus on longitudinal data &
\href{https://atrihub.shinyapps.io/power/}{yes} & no & limited set of
study designs with two levels & Iddi and C Donohue
(\citeproc{ref-iddiPowerSampleSize2022}{2022}) \\
mixedpower & Simulation-based power analysis; based on `simr' package &
no & yes & none & Kumle et al.
(\citeproc{ref-kumleEstimatingPowerGeneralized2021}{2021}) \\
mlmpower & Simulation-based power analysis; missing data mechanisms & no
& no & limited to two levels & Enders et al.
(\citeproc{ref-endersSimpleMonteCarlo2023}{2023}) \\
pamm & Simulation-based power analysis; based on `lme4' package & no &
no & none & Martin et al.
(\citeproc{ref-martinMeasuringIndividualDifferences2011}{2011}) \\
pass.lme & Formula-based power analysis; limited documentation & no & no
& none & Marco Chak Yan YU
(\citeproc{ref-marcochakyanyuPassLmePower2019}{2019}) \\
PowerAnalysisIL & Simulation-based power analysis; focus on longitudinal
data &
\href{https://github.com/ginettelafit/PowerAnalysisIL?tab=readme-ov-file\#shiny-app-and-r-package-to-perform-power-analysis-to-select-the-number-of-participants-in-intensive-longitudinal-studies}{yes}
& no & limited set of study designs with two levels & Lafit et al.
(\citeproc{ref-lafitSelectionNumberParticipants2021}{2021}) \\
powerlmm & Simulation-based power analysis; missing data mechanisms;
focus on longitudinal data &
\href{https://github.com/rpsychologist/powerlmm?tab=readme-ov-file\#launch-interactive-web-application}{yes}
& no & limited set of study designs with two or three levels & Magnusson
et al.
(\citeproc{ref-magnussonConsequencesIgnoringTherapist2018}{2018}) \\
simglm & Simulation-based power analysis; missing data mechanisms &
\href{https://simglm.brandonlebeau.org/reference/run_shiny.html}{yes} &
yes & limited to two or three levels & LeBeau
(\citeproc{ref-lebeauSimglmSimulateModels2017}{2017}) \\
simr & Simulation-based power analysis; based on `lme4' package & no &
yes & none & Green and MacLeod
(\citeproc{ref-greenSIMRPackagePower2016}{2016b}) \\
summary\_statistics based\_power & Summary-statistics-based power
analysis; based on `pwr' package &
\href{https://koumurayama.shinyapps.io/summary_statistics_based_power/}{yes}
& yes & limited set of study designs with two levels & Murayama et al.
(\citeproc{ref-murayamaSummarystatisticsbasedPowerAnalysis2022}{2022}) \\
WebPower & Formula-based power analysis; based on `longpower' package;
focus on longitudinal data &
\href{https://webpower.psychstat.org/wiki/start}{yes} & no & limited set
of study designs with two levels & Zhang and Yuan
(\citeproc{ref-zhangPracticalStatisticalPower2018}{2018}) \\
\end{longtable}

{\vspace{-20pt}
\noindent \emph{Note.} GUI available = whether a graphical user interface is provided (usually a Shiny app that is available online or can be downloaded to run locally), GLMM support = whether power analysis can be performed for generalized linear mixed model (otherwise only linear mixed models are supported). Design limitations = whether only a limited selection of study designs can be specified (otherwise arbitrary designs within the model class are supported).}
...
outlined in Table~\ref{tbl-rpackages}. Circumstances under which
tailored simulation-based sample size planning is necessary include 1)
complex study designs, 2) complex statistical hypotheses, 3) planning
for precision, and 4) lack of prior studies or pilot data.
...
Table~\ref{tbl-rpackages} can handle combined hypotheses and only some
...
cannot use the software packages outlined in Table~\ref{tbl-rpackages}.
...
CIs are computed for each simulated dataset, and their expected width is
estimated.
...
followed by the practical application based on a case study. All code in
this manuscript and simulation results are available on the Open Science
Framework (\url{https://osf.io/dhwf4/}) and on our tutorial website
...
tool to decide whether the specified standard deviations of the subject
and item random intercepts (\(\sigma_S\) and \(\sigma_I\)) are
...
probabilities shown in Table~\ref{tbl-probtable}.
...
takes about one hour on a MacBook Pro from 2020 with M1 chip and 16 GB
...
experts to students, the frequency of whether advice is presented at
all, and whether it is correct or incorrect.
...
\bibitem[\citeproctext]{ref-lebeauSimglmSimulateModels2017}
LeBeau, B. (2017). \emph{Simglm: {Simulate Models Based} on the
{Generalized Linear Model}} (p. 0.8.9). Comprehensive R Archive Network.
\url{https://doi.org/10.32614/CRAN.package.simglm}
...
\bibitem[\citeproctext]{ref-marcochakyanyuPassLmePower2019}
