...
Yet, these simple statistical methods are often not sufficient for research questions requiring more complex designs and methods. However, software-based power analysis tools cannot be used or adjusted for those.
<!-- TK: FP do you like the previous section how it is now? Please adjust, if required -->
...
Based on the need for well-powered experimental research using GLMMs and the lack of tools to conduct corresponding power analyses, in this tutorial paper, we present a case study that serves as a practical demonstration of how to perform a simulation-based a priori power analysis with GLMMs. Thereby, we aim to equip researchers with the tools needed to simulate their own data and determine appropriate sample sizes for their own research.
...
$$ If the dependent variable (i.e., diagnostic decision)<!-- SG: not sure if diagnostic accuracy as a term is useful as a binary dependent variable since it is usually calculated as a percentage over many cases.). inserted "diagnostic decision". TK: i like it  --> $Y$ is a binary variable with values $0$ (i.e. inaccurate), or $1$ (i.e., accurate), the conditional expected value is equivalent to the probability: $$
...
<!-- SG Table number, title and note are missing. TK: FP an you please insert this in final manuscript? -->
