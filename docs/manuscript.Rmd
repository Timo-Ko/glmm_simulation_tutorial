...
authornote: |
  ...
  All materials (reproducible manuscript incl. R code, simulation results) are available in the project's repository on the Open Science Framework (OSF): https://osf.io/dhwf4/
abstract: |
...
- Lakens ebook evtl. Andy Field für die Basics?
- R for data science, Bruine article, faux documentation für die R skills
- kumle power tutorial und irgendwelche paper (am besten aus AMPPS) zu GLMMs
- what is your estimand? paper für das estimand topic
...
Although many pragmatic justifications are possible [@lakensSampleSizeJustification2022], the ideal is often considered to determine a suitable sample size a priori (i.e., before the study is conducted) based on some meaningful computation to assure that the study will be able to fulfill its purpose.
...
In empirical research relying on hypothesis testing, the most common strategy for determining an adequate sample size is based on statistical power [@lakensSampleSizeJustification2022].
...
As study designs become more complex, psychological researchers require more sophisticated statistical models to capture the nuanced relationships and grouping structures introduced by their study designs [@yarkoniGeneralizabilityCrisis2022].
GLMMs (also called multilevel models) are gaining increasing popularity in analyzing data because they offer a flexible framework for handling data with outcome variables that are not normally distributed (e.g., categorical outcomes) while accounting for both fixed and random effects [@fahrmeirRegressionModelsMethods2021]. 

<!-- intro into GLMMs -->
GLMMs are an extension of LMMs (Linear Mixed Models), which are, in turn, extensions of linear regression models that account for correlated data including hierarchical structures  [@fahrmeirRegressionModelsMethods2021]. 
...
For simpler statistical models, like t-tests, ANOVA, and linear regression, with common study designs (e.g., mean comparison between two groups), user-friendly software for a priori sample size planning is readily available [@faulStatisticalPowerAnalyses2009; @champelyPackagePwr2018; @kelleyObtainingPowerObtaining2003; @kelleySampleSizePlanning2006]. 
...
Power analysis methods for multilevel models can be categorized into formula-based methods and simulation-based methods [@murayamaSummarystatisticsbasedPowerAnalysis2022].
...
Currently available formula-based software packages for power analysis with multilevel models often do not include GLMMs or are limited to very simple designs [@westfallStatisticalPowerOptimal2014; @murayamaSummarystatisticsbasedPowerAnalysis2022], making it necessary to build data simulations tailored specifically to the study design. 
A number of tutorials have been published describing how to perform such simulation-based power analysis for multilevel models [@arendStatisticalPowerTwolevel2019; @lafitSelectionNumberParticipants2021; @zimmerSampleSizePlanning2022; @brysbaertPowerAnalysisEffect2018; @kumleEstimatingPowerGeneralized2021; @debruineUnderstandingMixedEffectsModels2021]. 
However, most of these tutorials focus on linear mixed models (LMMs) and the most common designs [but see @kumleEstimatingPowerGeneralized2021 for a tutorial that also covers more advanced settings]. This narrow focus provides limited guidance for researchers faced with more complex study designs, especially when little prior knowledge about plausible effect sizes is available [see the discussion in @kumleEstimatingPowerGeneralized2021]. 
...
# When do we profit from tailored data simulation?
...
### Data generating processes beyond standard GLMMs
...
### Directed hypotheses and testing against values other than 0
...
If the global hypothesis $H_0$ is combined with *OR*, the p-values of the single hypotheses must be corrected for multiple testing to avoid $\alpha$-inflation for the global hypothesis.
However, if the global hypothesis $H_0$ is combined with *AND*, a correction for multiple testing is not necessary but rather a mistake that unnecessarily reduces the power of the global hypothesis test.
...
Although the details differ depending on the specific study characteristics, each tailored simulation based sample size planning requires a series of steps and decisions that we will outline in this section.
...
The theoretical estimand is a unit specific quantity defined outside of any statistical model.
...
When the theoretical estimand has been specified, the next step is to define the empirical estimand, which reexpresses the theoretical estimand in a way that can be estimated based on the observed data that will be collected in the planned study.
For many common research questions in psychology, the empirical estimand can be expressed as a statistical quantities that can be estimated with a regression model.
Note however, that this is not always possible and that the literature on causal inference includes many estimation strategies beyond regression.
...
When a regression model is used for estimation, this includes decisions like which statistical framework and package is used to fit the model, and how to compute hypothesis tests or confidence intervals.
...
In the investigated AI-enabled diagnostic decision support system, an AI model can provide initial diagnostic advice, which can be used as guidance by the humans require to make the final diagnostic decision.
The research goal is to validate the effectiveness of the AI-enabled advice.
We consider the AI-enable as effective, if the following pattern holds, which we will here describe verbally:

*We expect that for BOTH expert radiologists and students under training, correct AI-advice leads to a higher probability of accurately diagnosing a CT scan compared to no AI-advice presented, AND, we expect that for BOTH experts and non-experts, incorrect advice leads to a lower probability of accurately diagnosing a CT scan compared to no advice presented.*

I becomes clear that our theoretical estimand is be a causal quantity, based on individual quantities define outside of any statistical model.
To explain why, we focus on only on part of the research question concerning the comparison between correct AI-advice and no AI-advice for expert radiologists.
...
$$
\begin{aligned}
& P(\text{correct diagnosis} | \text{correct advice, a specific expert, a specific scan}) \\
& \quad - P(\text{correct diagnosis} | \text{no advice, a specific expert, a specific scan})
\end{aligned}
$$
However, our verbal description is not precise enough yet, because the above quantity exists and may differ for each combination of expert and scan.
We must specify how we want to aggregate the quantity across different experts and scans.
If we would compute the average of our individual quantity across all experts and scans in the target population, the result would be called the average treatment effect (ATE) in the population of experts.
Although often debated whether this is a good idea, psychological research often focuses on a different quantity.
Instead of averaging over experts and scans, we focus in a hypothetical *typical* expert and a *typical* scan, where *typical* is usually defined as an average score on all attributes of the expert or scan.
With this definition, the theoretical estimand in our above example can be expressed as:

$$
\begin{aligned}
& P(\text{correct diagnosis} | \text{correct advice, an average expert, an average scan}) \\
& \quad - P(\text{correct diagnosis} | \text{no advice, an average expert, an average scan})
\end{aligned}
$$
In total, our theoretical estimand consists of four expressions like the one above, the remaining three are:

$$
\begin{aligned}
& P(\text{correct diagnosis} | \text{no advice, an average expert, an average scan}) \\
& \quad - P(\text{correct diagnosis} | \text{wrong advice, an average expert, an average scan})
\end{aligned}
$$
$$
\begin{aligned}
& P(\text{correct diagnosis} | \text{correct advice, an average student, an average scan}) \\
& \quad - P(\text{correct diagnosis} | \text{no advice, an average student, an average scan})
\end{aligned}
$$

$$
\begin{aligned}
& P(\text{correct diagnosis} | \text{no advice, an average student, an average scan}) \\
& \quad - P(\text{correct diagnosis} | \text{wrong advice, an average student, an average scan})
\end{aligned}
$$
...
While a theoretical estimand is defined outside of any statistical model and be impossible to observe directly, an empirical estimand is a quantity that can be estimated based on observed data.
...
For our example, each probability expression in our theoretical estimand can be modeled with a GLMM.
...
We will discuss strategies for how these values can be obtained later.
...
<!-- The goal is to understand how AI advice influences medical decision-making. -->
Participants, radiologists (task experts; 1/4 of participants) and students/interns (non-task experts; 3/4), review a series of head computer tomography (CT) scans to assess the presence of a bleeding. 
To support their decision-making, an AI model provides initial diagnostic advice (2/3 of cases), which can be used as guidance by the participants.
In the control condition (1/3), no AI advice is presented, meaning that the participants have to read the CT scan without any support.
When AI advice is given, this advice can be either correct (4/5 of cases) or incorrect (1/5).
...
## Strategies on specifying the population parameters

### THEORY

As discussed earlier, tailored simulation based sample size planning is especially relevant when studies have to be planned in the absence of previous studies with a similar design or any pilot data.
...
In contrast to non-hierarchical linear regression, common heuristics based on standardized effect sizes are less useful or not even available.
Our strategies to specify population parameters will require access to domain knowledge from domain experts.
Because most domain knowledge of domain experts can only be expressed in unstandardized measurement units of a specific application, we argue that unstandardized effect sizes are usually preferred over standardized effect sizes for tailored simulation based sample size planning.
This requires new strategies on how the data generating process implied by certain values of population parameters can be quantified or visualized in an intuitive way that enables a calibration of population parameters based on the available domain knowledge of domain experts.
...
In this literature, there have been developed some strategies to monitor the plausibility of (prior) model assumptions, that can be adapted for our purposes.
...
Ideally, one would rely on meta-analytic results or conclusive data from pilot studies. However, these are sometimes not readily available. All parameter values in our present case study have been determined based on results from related study designs from the literature. Additionally, we had repeated discussions with our affiliated domain experts in radiology to check whether our assumptions seems plausible. 

We now outline the strategies we used to determine plausible parameter values:
...
The most simple interpretation works by imagining a subject with average ability ($u_{0s} = 0$) responding to an item (i.e., CT scan) with average difficulty ($u_{0i} = 0$).
...
\end{aligned}
$$
We can revert this perspective by choosing plausible probability values and deriving the parameter values implied by these probabilities (that a subject with average ability solves an item with average difficulty) for each experimental condition.
...
apa_table(
  ...
  , col.names = c("Experimental condition",
    "$P(Y=1|\\mathbf{X=x}, \\mathbf{U} = \\mathbf{0})$",
    "Implied equation")
  , caption = "Assumed probabilities that an average subject solves an average item in each experimental condition."
...
For most GLMMs, the interpretation of model parameters are quite complicated which makes it difficult to specify plausible values for a specific application.
However, the mathematical structure of GLMMs determines which patterns in data would be produced by the model, if a specific set of values for the population parameters would be specified.
...
For example, domain experts might not be able to specify plausible values for the $\beta$ coefficients in a logistic regression model (which are measured on the log-odds scale).
...
For example, we determined the $\beta$ parameters in a way that corresponds to a desired conditional probability of $0.95$, that an expert with average ability solves an item with average difficulty when presented with correct advice.

However, even if the model were true, we would not observe that 95% of experts responding to items presented with correct advice from a big sample of subjects drawn from their natural distribution of ability and items drawn from their natural distribution of difficulty.
How much this estimated probability differs from the model implied probability of an average expert and an average items depends on the standard deviation of the random subject and random item intercepts (the two quantities are only equal if both standard deviations would be zero).
We can use the model implied observed proportion of correct diagnoses in each experimental condition as an insightful descriptive statistics to determine plausible values for the random effect standard deviations.

We will simulate a large dataset (for which the observed values of the descriptive statistic will be close to their model implied true values) and simply compute the relative frequency of correct diagnoses for each experimental conditions.
...
We can use these descriptive statistics to judge whether our chosen values for the random effect standard deviations would produce data that align with out domain expertise.

However, in our example they are not informative enough to determine a final set of plausible parameter values.
...
The inequality of conditional and marginal effects in GLMMs [@fahrmeirRegressionModelsMethods2021] makes their interpretation more difficult. One must be careful when specifying parameter values based on previous studies or pilot data that use the marginal interpretation (e.g., a pilot study providing an estimate of how often neurologists make an accurate diagnosis based on brain scans). However, this does not mean that we cannot use the marginal interpretation (average probability across persons and items) to inform plausible parameter values: When parameter values have been selected, we can compute the implied marginal distributions and compare this information to our domain knowledge. Then, we can iteratively adjust the parameter values until we are satisfied with the implied distributions.

In the last section, we simulated a large simulated dataset and computed a descriptive statistic, the relative frequencies of correct diagnoses, for each experimental condition.
We will now use the model implied probability of each simulated data entry (y_prob) to visualize the whole model based implied marginal distribution of correct diagnoses for each condition.

(ref:margdist1-caption) Marginal distributions including means, 66% and 95% confidence intervals for all experimental conditions.

```{r margdist1, fig.cap="(ref:margdist1-caption)"}
library(tidyverse)
library(ggdist)
dat <- simulate(n_subjects = 3000, n_items = 3000, sd_u0s = 0.5, sd_u0i = 0.5)
dat %>% 
  mutate(condition = fct_cross(
    factor(expert), factor(advice_present), factor(advice_correct))) %>%
  mutate(condition = fct_recode(condition,
...
dat %>% 
  ...
  mutate(condition = fct_recode(condition,
    ...
    "no expert, correct advice" = "0:1:1", "expert, correct advice" = "1:1:1")) %>%
  group_by(condition) %>%
  summarize(freq_y_1 = sum(y_bin) / n())
```
...
(ref:margdist2-caption) Marginal distributions including means, 66% and 95% confidence intervals for all experimental conditions while setting the standard deviation of item random intercepts to 0.01.

```{r margdist2, echo=FALSE, fig.cap="(ref:margdist2-caption)"}
...
(ref:margdist3-caption) Marginal distributions including means, 66% and 95% confidence intervals for all experimental conditions while setting the standard deviation of subject and item random intercepts to 3.

```{r margdist3, echo=FALSE, fig.cap="(ref:margdist3-caption)"}
...
All parameter values in our present case study have been determined from results from related prior work and repeated discussions with our affiliated domain experts in Radiology to validate our assumptions.
Initially, we reviewed the literature to establish a reasonable baseline performance rate for examining head CT scans for ICH. Existing studies indicate that radiologists typically demonstrate high accuracies, often exceeding or hovering around 90%, while interns have been shown to perform below 80%, and medical students fall even shorter.
...
From their findings, we inferred that both experts and non-experts would benefit from correct and suffer losses from incorrect advice.
However, the magnitude of these effects would be substantially greater for non-experts, given their demonstrated reliance on advice compared to experts.
...
In such more complicated scenarios, the final model specification might also depend on the next decision on specifying the theoretical estimand.
...
The lme4 package includes a function called `simulate` that allows researchers to simulate the dependent variable based on the same model formula used for model fitting, enabling simulation-based power analyses and other related analyses. 
...
We simulate data according to our model, in which 100 subjects respond to 50 items (we use `set.seed` to make the simulation reproducible). However, for the sake of the exercise, we can imagine that this would be real data resulting from our future experiment and think about how we would analyze this data.
...
This decision is then followed by selecting the specific statistical method that shall be applied to compute the hypothesis test(s) or confidence interval(s) (e.g., compute hypothesis tests and confidence intervals with the marginaleffects R package using the delta method).

### PRACTICE

For our case study, we investigate the following research question:
*We expect that for both experts and non-experts, correct advice leads to a higher probability of accurately diagnosing a CT scan compared to no advice presented, AND, we expect that for both experts and non-experts, incorrect advice leads to a lower probability of accurately diagnosing a CT scan compared to no advice presented.*
...
The next translation step is to specify how these probabilites can be estimated within the framework of our concrete GLMM.

The first of the four statements was: 
...
\end{aligned}
$$
In the context of our GLMM, the corresponding model based quantity is:
...
<!-- $$ -->
<!-- \begin{aligned} -->
<!-- & \text{omv}[\beta_0 + \beta_a \cdot 1 + \beta_c \cdot 1 + \beta_e \cdot 1 + \beta_{ea} \cdot 1 \cdot 1 + \beta_{ec} \cdot 1 \cdot 1]\\ -->
<!-- & \quad - \text{inverse_logit}[\beta_0 + \beta_a \cdot 0 + \beta_c \cdot 0 + \beta_e \cdot 1 + \beta_{ea} \cdot 1 \cdot 0 + \beta_{ec} \cdot 1 \cdot 0] = \\ -->
<!-- & = \text{inverse_logit}[\beta_0 + \beta_a  + \beta_c + \beta_e + \beta_{ea} + \beta_{ec} ]\\ -->
<!-- & \quad - \text{inverse_logit}[\beta_0 + \beta_e ] \\ -->
<!-- \end{aligned} -->
<!-- $$ -->

When model parameters are not known, the corresponding parameter estimates are plugged into the equation to compute an estimate of the contrast instead.

In general, we can inspect the estimates for all model parameters with the `summary` command:
...
Unfortunately, there is no single model parameter that corresponds with our contrast of interest.
the model parameters in a binomial GLMM are hard to interpret because 1) the $\beta$ parameters are connected to the modeled probability via the non-linear inverse-logit link, and 2) we also have to consider the random effects.
When we are interested in a subject with average ability ($u_{0s} = 0$) responding to an item (i.e., CT scan) with average difficulty ($u_{0i} = 0$), the model implied probability that such a person solves such an item accurately is given by: 
$$
\begin{aligned}
P(Y=1|\mathbf{X=x}, \mathbf{U} = \mathbf{0}) = \\
= \text{inverse\_logit}[&\beta_0 + \beta_a \cdot advice\_present_{si} + \beta_c \cdot advice\_correct_{si} + \beta_e \cdot expert_s + \\
...
\end{aligned}
$$
One could use such reasoning, to determine the exact formula needed to compute our contrast of interest.
However, this would be very tedious and we can use R to compute this contrast for us without doing the math.

We can use the predict function of the lme4 package to compute the predicted probability for a correct diagnosis based on our model, plugin the two sets of predictor values, and compute the difference between the two estimated probabilities.

```{r}
pred_df <- data.frame(advice_present = c(1, 0), advice_correct = c(1, 0), expert = c(1, 1))
pred_df
...
The argument `type = "response"` specifies that predictions are made on the probability scale, while `re.form = NA` sets all random effects to 0.

We could use this method to compute point estimates for all four contrast that are part of our estimand.
...
The marginaleffects package [@R-marginaleffects] is a very flexible package to compute hypothesis tests and confidence intervals for contrasts with a variety of statistical models, including GLMM estimated with lme4.

First, we compute estimated probabilities for all experimental conditions in our experiment with the `predictions` function:
...
Note that the result also contains the two missing by design conditions that are never observed in the actual study ($advice\_present = 0, advice\_correct = 1, expert = 1$ and $advice\_present = 0, advice\_correct = 1, expert = 0$), and we should never interpret those.

Next, we use these predictions to compute the four specific contrasts that are part of our estimand.
For this we must specify which rows in `probs` have to be subtracted from each other.
We will use the `hypotheses` function to compute our four contrasts of interest together with hypothesis tests and confidence intervals.
...
The expression `"b8 = b2"` is special syntax to subtract the estimate in row number 8 from the estimate in row number 2.
The argument `equivalence = c(0, 0)` can be used to compute one-sided p-values testing whether the constrast in the population is smaller than 0 (`p (NonSub)` column) or greater than zero (`p (NonInf)` column).
We computed the contrasts in a way that we expect positive values for all four of them.

#### Hypothesis testing

If we are interested in hypothesis testing in our case study, we test a combined null hypothesis that consists of four separate null hypotheses:
...
\end{aligned}
$$
The combined null hypothesis should only be rejected if **all** individual null hypotheses are rejected [i.e., intersection-union setting; @dmitrienkoTraditionalMultiplicityAdjustment2013].
In such cases, the error probabilities do not accumulate, and we would waste power when correcting for multiple tests.

With a standard significance level of $\alpha = 0.05$, we would not reject all four null hypotheses (the p-value in the `p (NonInf)` column for the third hypothesis is not significant) and therefore also not reject the combined null hypothesis for this simulated dataset.
...
If we are interested in parameter estimation in our case study, we focus on the two-sided confidence intervals of the four contrasts of interest.
...
Note that due to the constrained range of the probability scale, the width of the confidence interval differs between the four contrasts.
...
<!-- With the marginaleffects package [@R-marginaleffects], we can easily compute predictions for all observations in the dataset based on the fitted GLMM (including all fixed **and** random effects), and plot the average probability with confidence intervals for each experimental condition in Figure \@ref(fig:marginaleffects): -->

<!-- (ref:marginaleffects) Marginal distributions including means and 95% confidence intervals for all experimental conditions computed with the marginaleffects package. -->

<!-- ```{r marginaleffects, message=FALSE, fig.cap="(ref:marginaleffects)"} -->
<!-- library(marginaleffects) -->
...
Conducting the previous steps enable the analyst to 1) simulate a dataset, 2) estimate a GLMM, and 3) compute hypothesis tests or confidence intervals for estimands of interest, mirroring the analysis that will later be performed for the actual dataset of the planned study.
...
On a conceptual level, we first require a function, that takes as input the sample size and the full set of population parameter.
...
When planning for precision, the function should instead return the width of the confidence interval(s) of interest.
Secondly, we must run this function repeatedly with the same sample and population parameters.
Because even fitting GLMMs with frequentist methods can quickly become time-consuming, it is recommended to use parallel computations, that is running simulations on multiple cores of the computer at the same time to reduce total computation time.
Thirdly, the results of the repeated simulation must be collected and aggregated.
When planning for power, we compute the relative frequency of a significant p-value across repeated simulations.
When planning for precision, we compute the average width of the confidence interval.
Lastly, we have to repeat the complete simulation for different sample sizes, to determine how big the sample must be in order to achieve the targeted statistical power or the expected width of the confidence interval.

### PRACTICE

With all these considerations addressed, we are finally ready to run our simulation-based sample size planning analyses to plan for power and precision. Wrapping the `simulate` function already constructed earlier, the helper function `sim_and_analyse` performs all previous steps (simulate a dataset, fit a GLMM, compute p-values and confidence intervals) in a single command.
...
The result of the computation is a data frame that contains the p-values and confidence intervals of all specified contrasts for each simulated dataset.
...
```{r finalpwr, fig.cap="Simulation-based power estimates including 95% confidence interval of the case study for different numbers of subjects and items, based on a significance level of 0.05."}
library(binom)
alpha <- 0.05
power <- sim_result %>%
  ...
  summarise(
    power = mean(`p.value.noninf_b1=b5` < alpha & `p.value.noninf_b8=b2` < alpha &
      `p.value.noninf_b2=b6` < alpha & `p.value.noninf_b7=b1` < alpha), 
    n_sig = sum(`p.value.noninf_b1=b5` < alpha & `p.value.noninf_b8=b2` < alpha &
      `p.value.noninf_b2=b6` < alpha & `p.value.noninf_b7=b1` < alpha),
    n = n(),
...
Based on our simulation outcomes, we compute the precision estimate for each combination of `n_subjects` $\times$ `n_items` (including 95% confidence intervals) and visualize the results with the following code.

```{r finalprecision, fig.cap="Simulation-based precision estimates (expected width of confidence intervals) including 95% confidence interval of the case study for different numbers of subjects and items, based on a confidence level of 0.95."}
...
precision %>%
  ...
  geom_text(aes(label = sprintf("%.2f \n [%.2f; %.2f]", 
    ...
    color = "white", size = 6) +
  scale_fill_viridis_c(limits = c(0, 0.3), direction = -1) + guides(fill = guide_legend(reverse=FALSE))
```
...
# THIS WAS THE OLD CASE STUDY CHAPTER
...
We can specify and test hypotheses like these with the multcomp package [@hothornSimultaneousInferenceGeneral2008] as follows:
...
saveRDS(sim_design, file = "results.rds")
```
...
sim_design <- readRDS(file = "results.rds")
```

```{r finpwr, fig.cap="Simulation-based power estimates including 95% confidence interval of the case study for different numbers of subjects and items, based on a significance level of 0.05.", eval=FALSE}
library(binom)
alpha <- 0.05
power <- sim_design %>%
  group_by(n_subjects, n_items) %>% 
...
As should be the case, power estimates in Figure \@ref(fig:finpwr) increase with both the number of subjects and the number of items. The confidence intervals indicate how precisely power was estimated by our simulation. Higher precision (which would be reflected in narrower confidence intervals) could be obtained by increasing the number of repetitions (`rep`) in the simulation. In practice, data simulations are often run multiple times with adjusted combinations of sample sizes. When running for the first time, it might be revealed that power is way too low (or much higher than required) for some combinations of `n_subjects` and `n_items`. When narrowing down the best combination that achieves sufficient power while at the same time striking a good balance of how many subjects and items are practically feasible, later rounds of data simulation will typically include a smaller grid of sample sizes combined with a higher number of repetitions. This will assure high precision for the final power estimates, which are then used for the sample size justification of the future study.

Much has been written on the optimal amount of power to target in empirical research. The most prominent heuristic is to target a power of 0.8 (when combined with a type I error rate of $\alpha = 0.05$), but depending on the research goals of the study, there are often good reasons to move away from this standard depending on the research goals and resource constraints  [@lakensJustifyYourAlpha2018; @lakensSampleSizeJustification2022]. When target power has been specified, the number of subjects and the number of items in our study design can be traded against each other based on practical considerations. For the sake of the example, let the targeted power be indeed about 0.8, using an $\alpha$ of 0.05 to detect an effect of the expected size implied by our data simulation. This could be achieved by collecting data from 200 subjects (about 25% of which will be experts), each completing the same 50 items (with advice present in about 67% of cases, which is correct in about 80% of cases with present advice). If collecting data from 200 subjects is not feasible, an alternative would be to recruit 150 subjects but increase the length of the experiment to over 70 items. However, 70 items might take too long to complete for the radiologists participating in the study, who have a busy schedule. The simulation suggests that it might also be possible to plan a shorter experiment with only 30 items if it is feasible to recruit an even higher number of subjects (\> 250, to be determined by additional rounds of power analysis). Design parameters that also affect power, and which could be investigated in the simulation to find a more optimal trade-off, are the ratio of experts, the frequency of whether advice is presented and whether it is correct.

## Expected effect size vs. smallest effect size of interest: sensitivity analysis
...
1.  The growing need for simulation-based sample size planning in experimental research: In order to conduct informative research using varying complex experimental designs with GLMMs formula-based heuristics and user-friendly software tools for a priori power analysis are often not suitable. Therefore, simulation-based power analysis is becoming increasingly needed since it provides experimental researchers with a tailored approach to estimating required sample sizes before data collection. 

2.  Managing data simulations more easily with discrete predictor variables: Simulation-based sample size planning becomes more manageable when all predictor variables are discrete (like in the presented case study) and fixed by the study design. 
...
3.  Teaching data simulation skills: The ability to conduct simulation-based sample size planning is a valuable skill that should be taught to experimental researchers. By incorporating such training into research methods courses and workshops, researchers can gain a deeper understanding of statistical power and improve the quality of their experimental designs. 
