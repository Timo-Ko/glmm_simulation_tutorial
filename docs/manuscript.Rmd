...
Therefore, thorough experiments are needed to study human-AI interaction to understand how people perceive and respond to these novel technologies. 
When conducting experimental research in this emerging field, it is essential to determine an appropriate sample size to ensure that the results obtained are both statistically significant and reliable, and use appropriate statistical models to analyze the data.

<!-- Some basics on statistical power, power analysis and why it is important -->
In empirical research relying on hypothesis testing, the most common strategy to determine appropriate samples size is based on statistical power. 
...
Moreover, existing tutorials often rely on heuristics for specifying variance components (e.g., standard deviation of random intercepts) or assume that meta-analytic results or data from pilot studies is available to determine plausible values for all model parameters.
In contrast, simulations are often required, necessitating assumptions for all parameters in the model.
Therefore, running sample size estimations is crucial for designing experiments that can provide meaningful insights into human-AI interaction.
...
In the present example, it would be more challenging to recruit task expert (i.e., radiologists) as there is only a limited amount of such people. Non-experts would be easier to recruit. The goal of the power simulation is to determine how many task experts and non-experts to recruit to achieve sufficient statistical power in our experiment. 
...
Therefore in this tutorial, we simulate data for GLMMs from first principles to assist applied researchers in better understanding all model assumptions, and then use lme4 to analyze the simulated data sets.
