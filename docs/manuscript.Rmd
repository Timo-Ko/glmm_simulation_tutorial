...
For our example, each probability expression in our theoretical estimand can be modeled with a concrete GLMM.
...
We simulate data for an experiment where the diagnostic performance of users of an AI-enabled diagnostic decision support system will be evaluated. 
...
This decision is then followed by selecting the specific statistical method that shall be applied to compute the hypothesis test(s) or confidence interval(s) (e.g., compute hypothesis tests with the marginaleffects R package, using the delta method).
...
We can specify and test hypothesis like these with the marginaleffects [@R-marginaleffects] package.

```{r}
library(marginaleffects)

hypotheses(fit, hypothesis = c(
  "`advice_present` + `advice_correct` + `expert:advice_present` +
  `expert:advice_correct` = 0",
  "`advice_present` + `advice_correct` = 0",
  "`advice_present` + `expert:advice_present` = 0",
  "`advice_present` = 0"
), equivalence = 0)
...
```{r}
probs <- avg_predictions(fit, variables = list(
  expert = 0:1, advice_present = 0:1, advice_correct = 0:1), 
