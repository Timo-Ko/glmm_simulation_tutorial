...
In addition, readers should have basic programming skills in R on how to simulate data and repeat these simulations in parallel to speed up computation.
While there are a myriad options for data simulation in R beyond the basic functionality in the base and stats packages, in our tutorial we use functions from the tidyverse and the faux package.
...
While it is not necessary to understand the technical details of how GLMMs are estimated, it is crucial to understand the structure of a basic GLMM (e.g., logistic regression with random intercepts).
...
- Missing data processes; data missing by design; dropout
...
- Planning for expected width of confidence intervals
    - The most interesting contrasts are usually on the scale of the outcome instead of the scale of the coefficients
    
### No prior studies or pilot data

- No prior studies or pilot data available
    - Detailed data simulation is necessary to identify plausible scenarios for effects sizes

## General steps in simulation-based sample size planning

Each tailored simulation based design analysis requires a series of steps and decisions.
...
- Specify estimand (conditional vs. marginal perspective)
