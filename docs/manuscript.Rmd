...
In line with this, power analysis represents the act of calculating the statistical power for a given true effect and sample size. 
...
In this paper, we present a case study that serves as a practical demonstration of how to perform a data simulation for a concrete study for which no meta-analytic results or conclusive data from pilot studies is available.
...
In this section, we describe the steps to conduct a data simulation and a power analysis for GLMMs with a specific experimental design in human-AI interaction research in mind in the form of a case study.

## The present case study

<!-- The present example, describe the study idea and design -->
In the case study, we simulate the data for a human-AI Interaction experiment where the diagnostic performance of users of a health care AI is to be evaluated. The goal is to understanding how AI advice influences medical decision-making. Participants, radiologists (task experts) and emergency doctors (non task experts) view head CT (computertomography) scans and evaluate if an intracranial bleeding is present. To support their decision making, an AI provides a suggestion for a medical diagnosis. This AI advice can be either either correct or incorrect. 
Participants' medical diagnosis (i.e., bleeding or no bleeding) can be correct or incorrect. 
Also, we have a control group with no AI advice present. This introduces missings by design that has an effect on our data simulations. 
...
For our data simulation and data analysis, we use the lme4 R package [@batesFittingLinearMixedEffects2015]. The lme4 package is a state-of-the-art tool for fitting frequentist GLMMs (for Bayesian GLMMs, the brms R package is currently the most prominent option [@burknerBrmsPackageBayesian2017]).
The lme4 package includes a useful function called "simulate" that allows researchers to simulate the dependent variable based on the same model formula used for model fitting, enabling power simulations and other related analyses.
...
When run for the first time, it might be revealed that power is way too low (or too high) for some combinations of `n_subjects` and `n_items`.
...
The most prominent heuristic is to target a power of 0.8 (when combined with a type I error rate of $\alpha = 0.05$), but depending on the research goals of the study, there are often good reasons to move away from this standard.
...
For the sake of the example, let the targeted power be indeed about 0.8 to detect an effect of the expected size.
...
However, 70 items might take to long to complete (especially for the expert radiologists who might have a busy schedule).
The simulation suggests that it might also be possible to plan an even shorter experiment with only 30 items, if it is feasible to recruit an even higher number of subjects (> 250, to be determined by additional rounds of power analysis).
...
However, if the true effect is indeed smaller, power will be lower the study might not be sufficiently informative.
...
An effect smaller than the SESOI would be considered too small to be interesting or practically meaningful, even if the effect is not actually zero.
