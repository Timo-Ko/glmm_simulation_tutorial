...
The theoretical estimand is a unit specific quantity defined outside of any statistical model [@lundbergWhatYourEstimand2021].
...
I becomes clear that our theoretical estimand is be a causal quantity, based on several individual quantities that are defined outside of any statistical model [@lundbergWhatYourEstimand2021].
...
This requires new strategies on how the data generating process implied by certain values of population parameters can be quantified or visualized in an intuitive way that enables a calibration of population parameters based on the available knowledge of domain experts.
...
In this literature, there have been developed some strategies to monitor the plausibility of (prior) model assumptions [@gelmanBayesianWorkflow2020], that can be adapted for our purposes.
...
How much this estimated probability differs from the model implied probability of an average expert and an average items depends on the standard deviations of the random intercepts (the two quantities are only equal if both standard deviations would be zero).
...
The already discussed inequality of conditional and marginal effects in GLMMs [@fahrmeirRegressionModelsMethods2021] makes their interpretation more difficult.
...
We will now use the model implied probability of each simulated data point (stored in the variable `y_prob`) to visualize the whole model based implied marginal distribution of correct diagnoses for each experimental condition.
