...
abstract: |
  Understanding how humans interact with articifical intelligence through experiments becomes increasingly relevant. However, human-AI interaction researchers lack the appropriate tools to conduct power and data analyses for the required complex study designs. In this work, we provide a tutorial on how to run customized power analyses using data simulation based on Generalized Linear Mixed Models (GLMMs). By providing code in a case study, we equip human-AI interaction researchers with the tools to simulate their own data and run analyses based on GLMMs. We discuss implications for human-AI interaction research and give an outlook.
...
However, most tutorials discuss only linear mixed models (LMMs) and focus on the most common designs, limiting the guidance available for researchers facing more complex study designs [REF].
...
In this section, we describe the steps to conduct a data simulation and a power analysis for GLMMs. In doing so, we provide a concrete example in the form of a case study.
...
LMMs and GLMMs are powerful statistical frameworks that handle complex data structures by incorporating both fixed and random effects. 
...
The lme4 R package is a state-of-the-art tool for fitting frequentist GLMMs (for Bayesian GLMMs, the brms R package is currently the most prominent option).
...
Therefore in this tutorial, we simulate data for GLMMs from first principles to assist applied researchers in better understanding all model assumptions, and then use lme4 to analyze the simulated datasets.
